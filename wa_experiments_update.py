...
telegram_send.send(messages=["\U000023F3 Epochs done - {time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())}."])

for hidden_size in hidden_sizes:
    overwrite_dict = {'hidden_size': hidden_size,
                      'log_path': f'./logs/wa_exp_pmnist_update/hidden_size/{hidden_size}/'}
    wa_pmnist_update(overwrite_dict)
telegram_send.send(messages=["\U000023F3 Hidden size done - {time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())}."])

for hidden_layer in hidden_layers:
    overwrite_dict = {'hidden_layers': hidden_layer,
                      'log_path': f'./logs/wa_exp_pmnist_update/hidden_layers/{hidden_layer}/'}
    wa_pmnist_update(overwrite_dict)
telegram_send.send(messages=["\U000023F3 Hidden layer done - {time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())}."])
...
for no_experience in no_experiences:
    ...
    wa_pmnist_update(overwrite_dict)
telegram_send.send(messages=["\U000023F3 No experiences done - {time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())}."])
...
for weighting_method in weighting_methods:
    ...
    wa_pmnist_update(overwrite_dict)
telegram_send.send(messages=["\U000023F3 Weighting methods done - {time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())}."])
